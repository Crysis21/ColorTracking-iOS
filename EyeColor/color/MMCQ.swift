...
open class MMCQ {
    ...
    public struct Color {
        ...
        init(r: UInt8, g: UInt8, b: UInt8, dominance: Float) {
    ...
    open class ColorMap {
        ...
        open func makePalette() -> [Color] {
            let totalCount = vboxes.reduce(0, {result, vbox  in
                return result + vbox.getCount() * vbox.getVolume()
            })
            var colors = [Color]()
            vboxes.forEach {
                var color = $0.getAverage()
                color.dominance = Float($0.getCount()*$0.getVolume()) / Float(totalCount)
                colors.append(color)
            }
            return colors
        }
